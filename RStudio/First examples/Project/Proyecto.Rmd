...
## Gráficos
```{r}
amBarplot(x = "Fecha", y = "CantidadCovid", data = BD, labelRotation = 90)
```
```{r}
ResumenUSDMes<-BaseDatos2 %>% group_by(Mes) %>% summarise(Compra=mean(CompraUSD),Venta=mean(VentaUSD))



ResumenUSDMes$Mes<-as.character(month(ResumenUSDMes$Mes, label = TRUE, abbr = FALSE))

amBarplot(x = "Mes", y = c("Compra", "Venta"), data = ResumenUSDMes, legend = TRUE, main = "Comparación de promedios de tipo de cambio del dolar",
        mainColor = "black", mainSize = 15, creditsPosition = "top-left", ylab = "Tipo cambio",xlab = "Meses")
```
